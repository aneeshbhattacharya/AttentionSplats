...
def render_with_local_transformer(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, local_transformer, scaling_modifier = 1.0, override_color = None):
    """
    Render the scene. 
    ...
    indices_of_visible = torch.where(visible_gaussians == 1)[0]
    visible_features = pc.get_semantic_feature[indices_of_visible]
    visible_positions = pc.get_xyz[indices_of_visible]
    indices_of_not_visible = torch.where(visible_gaussians == 0)[0]
    
    # print("Visible features shape in:", visible_features.shape) # S,B,F -> N, 1, 256 -> squeeze(1) -> N,256 -> unqueeze(0) -> 1,N,256
    # print("Visible positions shape:", visible_positions.shape)
    
    transformed_features = local_transformer(visible_positions.detach().clone(), visible_features.squeeze(1)) # This is now transformed features we have obtained
        
    new_semantic_features = torch.zeros_like(pc.get_semantic_feature)
        
    new_semantic_features[indices_of_visible] = transformed_features.unsqueeze(1)
    new_semantic_features[indices_of_not_visible] = pc.get_semantic_feature[indices_of_not_visible]
    ...
    rendered_image, feature_map, radii = rasterizer(
        ...
        shs = shs,
        colors_precomp = colors_precomp,
        semantic_feature = new_semantic_features, 
        opacities = opacity,
        scales = scales,
        rotations = rotations,
        cov3D_precomp = cov3D_precomp)
